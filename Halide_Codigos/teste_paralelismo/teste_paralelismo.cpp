...
float processImage(const std::string& inputPath, const std::string& outputPath, bool useParallelism) {
    ...
    if (useParallelism) {
        expansaoLinear.parallel(y).vectorize(x, 16);
        std::cout << "Paralelismo ativado." << std::endl;
    }
    else {
        std::cout << "Paralelismo desativado." << std::endl;
    }
...
int main(int argc, char** argv) {
    try {
        ...
        std::vector<float> mediasGeraisSemParalelismo;
        std::vector<float> mediasGeraisComParalelismo;

        std::cout << "Executando sem paralelismo..." << std::endl;
        for (const auto& entry : fs::directory_iterator(inputDir)) {
            std::string inputPath = entry.path().string();
            std::string outputPath = outputDir + "\\" + entry.path().filename().string();
            float duracao = processImage(inputPath, outputPath, false);
            mediasGeraisSemParalelismo.push_back(duracao);
        }

        std::cout << "Executando com paralelismo..." << std::endl;
        for (const auto& entry : fs::directory_iterator(inputDir)) {
            std::string inputPath = entry.path().string();
            std::string outputPath = outputDir + "\\" + entry.path().filename().string();
            float duracao = processImage(inputPath, outputPath, true);
            mediasGeraisComParalelismo.push_back(duracao);
        }

        float mediaSemParalelismo = std::accumulate(mediasGeraisSemParalelismo.begin(), mediasGeraisSemParalelismo.end(), 0.0f) / mediasGeraisSemParalelismo.size();
        float mediaComParalelismo = std::accumulate(mediasGeraisComParalelismo.begin(), mediasGeraisComParalelismo.end(), 0.0f) / mediasGeraisComParalelismo.size();

        std::cout << "Media de tempo sem paralelismo: " << mediaSemParalelismo << " ms" << std::endl;
        std::cout << "Media de tempo com paralelismo: " << mediaComParalelismo << " ms" << std::endl;

        return 0;
    }
    catch (const std::exception& e) {
        std::cerr << "Erro geral no main: " << e.what() << std::endl;
        return 1;
    }
}
