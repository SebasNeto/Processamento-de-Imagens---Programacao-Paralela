...
void filtroKVizinhosProximos(const Mat& imagem, Mat& imagemFiltrada, int tamJanela = 3, int k = 3) {
    ...
    for (int i = margem; i < altura - margem; ++i) {
        for (int j = margem; j < largura - margem; ++j) {
            for (int canal = 0; canal < canais; ++canal) {
                vector<pair<int, uchar>> distancias;
                for (int x = -margem; x <= margem; ++x) {
                    for (int y = -margem; y <= margem; ++y) {
                        uchar pixel = imagem.at<Vec3b>(i + x, j + y)[canal];
                        int distancia = abs(pixel - imagem.at<Vec3b>(i, j)[canal]);
                        distancias.push_back(make_pair(distancia, pixel));
                    }
                }
                sort(distancias.begin(), distancias.end());
                float soma = 0;
                for (int idx = 0; idx < k; ++idx) {
                    soma += distancias[idx].second;
                }
                imagemFiltrada.at<Vec3b>(i, j)[canal] = static_cast<uchar>(soma / k);
            }
        }
    }
}

vector<double> processarDiretorio(const string& input_dir, const string& output_dir, int tamJanela = 3, int k = 3) {
    ...
    for (const auto& entry : fs::directory_iterator(input_dir)) {
        if (entry.path().extension() == ".png" || entry.path().extension() == ".jpg" || 
            ...
            if (!imagem.empty()) {
                ...
                filtroKVizinhosProximos(imagem, imagemFiltrada, tamJanela, k);
...
void multiplasExecucoes(const string& input_dir, const string& output_dir, int tamJanela = 3, int k = 3, int execucoes = 1) {
    ...
    for (int execucao = 0; execucao < execucoes; ++execucao) {
        ...
        vector<double> tempos_execucao = processarDiretorio(input_dir, output_dir, tamJanela, k);
...
int main() {
    ...
    string output_dir = "C:/Users/Cliente/Downloads/base_dados/Saida_Python_Filtro_KNN";
